...
function buildSystemPrompt(userInstructions: string): string {
  return buildVapiSystemPrompt(userInstructions);
}
...
Your goal is to capture **exactly what transpired during the call** — as if taking professional call notes — using clear, complete sentences for each bullet point. Only include valuable and relevant information in the summary however. no need to include obvious and unnecessary aspects of the call.
